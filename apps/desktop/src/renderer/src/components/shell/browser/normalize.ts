/** Loopback authorities (localhost and subdomains plus bare IPv4/IPv6) default to http; the web defaults to https. */
const LOOPBACK_AUTHORITY_RE =
  /^(?:(?:[\w-]+\.)*localhost|\d{1,3}(?:\.\d{1,3}){3}|\[[\d.:a-f]+\])(?::\d+)?(?:[#/?]|$)/i
const SCHEME_RE = /^[a-z][\d+.a-z-]*:/i

/** Address-bar input → navigable URL (normalization rules). */
