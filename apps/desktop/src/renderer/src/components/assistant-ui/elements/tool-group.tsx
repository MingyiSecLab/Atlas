...
function ToolGroupRoot({
  ...
  children,
  ...props
}: ToolGroupRootProps): React.ReactNode {
...
export type ToolGroupTriggerProps = React.ComponentProps<typeof CollapsibleTrigger> & {
  count?: number
  active?: boolean
...
function ToolGroupTrigger({
  ...
  const defaultLabel = `${count} 项工具调用`
  ...
  return (
    <CollapsibleTrigger
      ...
      className={cn(
        'aui-tool-group-trigger group/trigger flex origin-left items-center gap-2 text-sm transition-[color,scale] active:scale-[0.98] cursor-pointer outline-none select-none',
    ...
    >
      {active && (
        <LoaderIcon
          ...
          className="aui-tool-group-trigger-loader size-3.5 shrink-0 animate-spin text-blue-500 [animation-duration:0.6s]"
        />
      )}
      <span
    ...
    >
      <div
        className={cn(
          ...
          'group-data-[variant=ghost]/tool-group-root:mt-1 group-data-[variant=ghost]/tool-group-root:gap-1.5',
